...
#include "Links/BLEPeripheral.h"

BLEPeripheralUi::BLEPeripheralUi(wxWindow *parent)
    : LinkUi(parent)
...
{
    delete dynamic_cast<BLEPeripheral *>(GetLink());
}

void BLEPeripheralUi::Disable()
{
...
wxtJson BLEPeripheralUi::DoSave() const
{
...
void BLEPeripheralUi::DoLoad(const wxtJson &json)
{
    BLEPeripheralParameterKeys keys;
    // clang-format off
...
void BLEPeripheralUi::DoRefreshDevice()
{
    if (GetLink()) {
        return;
    }
...
Link *BLEPeripheralUi::NewLink()
{
    return new BLEPeripheral();
}

void BLEPeripheralUi::SetupComboBox(wxComboBox *cb, const wxString &label, int row, wxWindow *parent)
{
